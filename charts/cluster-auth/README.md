...
[AppsCode License ProxyServer](https://github.com/bytebuilders/cluster-auth) - AppsCode License ProxyServer
...
$ helm repo update
$ helm search repo appscode/cluster-auth --version=v2024.2.25
$ helm upgrade -i cluster-auth appscode/cluster-auth -n kubeops --create-namespace --version=v2024.2.25
```
...
```bash
$ helm uninstall cluster-auth -n kubeops
```
...
The following table lists the configurable parameters of the `cluster-auth` chart and their default values.
...
```bash
$ helm upgrade -i cluster-auth appscode/cluster-auth -n kubeops --create-namespace --version=v2024.2.25 --set replicaCount=1
```
...
```bash
$ helm upgrade -i cluster-auth appscode/cluster-auth -n kubeops --create-namespace --version=v2024.2.25 --values values.yaml
```
